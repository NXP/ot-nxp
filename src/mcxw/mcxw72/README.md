# OpenThread on NXP MCXW72 Example
...
Download the MCXW72 SDK using the west tool.
...
```cmd
> cd <path-to-ot-nxp>
> sh script/build_mcxw72
```

After a successful build, the `elf` and `srec` files are found in `build_mcxw72/bin` and include FTD (Full Thread Device) and MTD (Minimal Thread Device) variants of CLI applications.
...
The image needed on the host side is the one generated in `build_mcxw72/bin` while the one needed on the NBU side can be found in the NXP-SDK at path - `middleware/wireless/ieee-802.15.4/bin/mcxw72/mcxw72_nbu_ble_15_4_dyn.sb3`.

### Flashing the NBU image

NBU image should be written only when a new NXP-SDK is released. [blhost tool](https://cache.nxp.com/secured/assets/downloads/en/device-drivers/blhost_2.6.7.zip?fileExt=.zip) can be used for flashing. Before writing the image, please make sure that MCXW72 is in bootloader mode by keeping the SW3 button (also named ISP) pressed while connecting the MCXW72 board to an USB port (unplug the board if already connected to an USB port):

```bash
C:\nxp\blhost_2.6.7> blhost.exe -p COM50 -- receive-sb-file .\mcxw72_nbu_ble_15_4_dyn.sb3
```

Please note that COM50 should be replaced with the COM port that corresponds to the MCXW72 device.

### Flashing the host image

Host image is the one found under `build_mcxw72/bin`. It should be written after each build process.
...
- Plug MCXW72 to the USB port (no need to keep the SW3 button pressed while doing this)
...
After this step, a debug configuration specific for the MCXW72 board was created. This debug configuration will
...
- Replace the path of the existing demo application with the path of the MCXW72 application:

```
Run -> Debug Configurations... -> C/C++ Application
```

![Debug MCXW72](../../../doc/img/mcxw/debug_mcxw.jpg)
