...
extern void *ot_ncp_host_get_command_buffer(void);

#ifndef NCP_HOST_COMMAND_LEN
...
int port_mbedtls_send(void *ctx, const unsigned char *buf, size_t len)
{
    ...
    cmd                        = ot_ncp_host_get_command_buffer();
    ...
    {
        ncp_e("cmd->header.size(%d) > NCP_HOST_COMMAND_LEN(%d)", cmd->header.size, NCP_HOST_COMMAND_LEN);
        return 0;
    }
    ...
    ret = ot_ncp_host_send_tlv_command();
    if (ret != 0)
    {
...
int ncp_process_encrypt_response(uint8_t *res)
{
    ...
    if (cmd_res->params.encrypt.action == NCP_CMD_ENCRYPT_ACTION_INIT)
    {
        if (!_mbedtls)
        {
            (void)OSA_TaskCreate((osa_task_handle_t)ncp_encrypt_handshake_thread, OSA_TASK(ncp_encrypt_handshake_task),
