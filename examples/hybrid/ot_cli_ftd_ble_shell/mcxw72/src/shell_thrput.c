...
#define mShellThrTxInterval_c ((mShellThrConnectionInterval_d + mShellThrConnectionInterval_d / 4U) / 3U) /* ms */
...
static gapAdStructure_t advScanStruct[3] = {
    ...
    {.adType = gAdShortenedLocalName_c, .length = 8, .aData = (uint8_t *)"THR_PER"}};

static gapAdvertisingData_t gThrAdvertisingData = {NumberOfElements(advScanStruct), (void *)advScanStruct};

static gapScanResponseData_t gThrScanRspData = {0, NULL};
...
static gapConnectionRequestParameters_t mConnReqParams = {.scanInterval       = gGapScanIntervalDefault_d,
...
static appScanningParams_t appThrputScanParams = {&mAppScanParams, gGapDuplicateFilteringEnable_c,
                                                  gGapScanContinuously_d, gGapScanPeriodicDisabled_d};

static appAdvertisingParams_t appThrputAdvParams = {&mAdvParams, &gAppAdvertisingData, &gAppScanRspData};
/************************************************************************************
*************************************************************************************
        ...
        else if (0 == strcmp((char *)argv[i], "-s") && (mThrShellRole == thrPeripheral))
        {
            ...
            if ((peerId != gInvalidDeviceId_c) && (peerId < gAppMaxConnections_c))
            {
                ...
                if ((newBufferSize > gAttMaxNotifIndDataSize_d(maxMtu)) &&
                    (newBufferSize <= gAttMaxNotifIndDataSize_d(gAttMaxMtu_c)) && (mThrWaitMtuExchange == FALSE))
                {
        ...
        else
        {
            ...
            else
            {
                ...
                result = GattDbDynamic_AddPrimaryServiceDeclaration(0, gBleUuidType128_c,
                                                                    (void *)uuid_service_throughput, &handle);
                if (result == gBleSuccess_c)
                {
                    /* Add characteristic */
                    result = GattDbDynamic_AddCharDescriptor(
                        gBleUuidType128_c, (void *)uuid_throughput_stream, (uint16_t)sizeof(aInitialValue),
                        aInitialValue, (gattAttributePermissionsBitFields_t)gPermissionFlagReadable_c, &handle);
                }
                ...
                if (result == gBleSuccess_c)
                {
                    ...
                    result = BluetoothLEHost_StartAdvertising(&appThrputAdvParams, ShellThr_AdvertisingCallback,
                                                              ShellThr_ConnectionCallback);
                }
                if (result != gBleSuccess_c)
                {
...
static void ShellThr_SendData(void *p)
{
    ...
    if ((pDummyTestData != NULL) && (gThroughputConfig[inParam.peerId].bTestInProgress))
    {
        ...
        else
        {
            ...
            result = GattServer_SendInstantValueNotification(inParam.peerId, value_throughput_stream,
                                                             gThroughputConfig[inParam.peerId].buffSz, pDummyTestData);
...
void ShellThr_ConnectionCallback(deviceId_t peerDeviceId, gapConnectionEvent_t *pConnectionEvent)
{
    ...
    case gConnEvtConnected_c:
    {
        ...
        FLib_MemCpy(&mConnectionParams, &pConnectionEvent->eventData.connectedEvent.connParameters,
    ...
    case gDeviceScanned_c:
    {
        ...
        if (FALSE == mFoundDeviceToConnect)
        {
            ...
            if (mFoundDeviceToConnect)
            {
                ...
                FLib_MemCpy(mConnReqParams.peerAddress, pScanningEvent->eventData.scannedDevice.aAddress,
...
static void ShellThr_StartThroughputTest(thrGapRoles_t role, deviceId_t peerId)
{
    ...
    /* MISRA Rule 10.8 - The value of a composite expression shall not be cast to a different essential type category or
    ...
    else /* GAP Peripheral */
    {
        ...
        if (mThrWaitMtuExchange == FALSE)
        {
            ...
            if (timerStatus == kStatus_TimerSuccess)
            {
                (void)TM_InstallCallback((timer_handle_t)gThroughputTestTimerId, ShellThr_TxTimerCallback,
                                         peerDeviceId.ptr);
                (void)TM_Start((timer_handle_t)gThroughputTestTimerId, (uint8_t)kTimerModeIntervalTimer,
                               txInterval.u32);
                shell_write("Throughput test started.\r\nSending packets...\r\n");
            }
            else
...
static void ShellThr_PrintReport(appCallbackParam_t pParam)
{
    ...
    if (gThrStatistics[inParam.peerId].notifRcv != 0U)
    {
        ...
        uint64_t rxDuration =
            (gThrStatistics[inParam.peerId].lastPacketTs - gThrStatistics[inParam.peerId].firstPacketTs) / 1000U;
