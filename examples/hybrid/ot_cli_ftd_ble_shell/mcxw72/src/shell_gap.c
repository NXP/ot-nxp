...
*************************************************************************************
************************************************************************************/
#if defined(BLE_SHELL_AE_SUPPORT) && (BLE_SHELL_AE_SUPPORT) && defined(BLE_SHELL_DBAF_SUPPORT) && \
    (BLE_SHELL_DBAF_SUPPORT)
...
void ShellGap_AdvertisingCallback(gapAdvertisingEvent_t *pAdvertisingEvent);

void ShellGap_ScanningCallback(gapScanningEvent_t *pScanningEvent);

void ShellGap_ConnectionCallback(deviceId_t peerDeviceId, gapConnectionEvent_t *pConnectionEvent);
...
static appScanningParams_t mAppScanParams = {&gAppScanParams, gGapDuplicateFilteringEnable_c, gGapScanContinuously_d,
                                             gGapScanPeriodicDisabled_d};

static appAdvertisingParams_t mAppAdvParams = {&gAdvParams, &gAppAdvertisingData, &gAppScanRspData};
...
static gapPeriodicAdvSyncReq_t mPeriodicSyncPeer = {.options.filterPolicy =
                                                        (gapCreateSyncReqFilterPolicy_t)gUseCommandParameters_c,
                                                    .SID             = 1U,
...
static appExtAdvertisingParams_t mAppExtAdvParams = {&gExtAdvParams,
...
shell_status_t ShellGap_Command(shell_handle_t shellHandle, int32_t argc, char **argv)
{
    ...
    if (argc >= 2)
    {
        ...
        {
            (void)SHELL_PrintfSynchronization(
...
static shell_status_t ShellGap_StartAdvertising(uint8_t argc, char *argv[])
{
    ...
    else
    {
        ...
        mIsCentral = FALSE;
        (void)BluetoothLEHost_StartAdvertising(&mAppAdvParams, ShellGap_AdvertisingCallback,
 ...
 ********************************************************************************** */
static bool_t ShellGap_AppendAdvData(gapAdvertisingData_t *pAdvData,
                                     gapAdType_t           type,
                                     char                 *pData,
                                     bool_t                fIsScanRsp,
...
static shell_status_t ShellGap_Connect(uint8_t argc, char *argv[])
{
    ...
    else
    {
        ...
        else
        {
            ...
            FLib_MemCpy(gConnReqParams.peerAddress, mScannedDevices[deviceId].aAddress, sizeof(bleDeviceAddress_t));
    ...
    case 2:
    {
        ...
        if (0 == strcmp((char *)argv[0], "-remove"))
        {
            ...
            if (result != kStatus_SHELL_Error)
            {
                ...
                if ((count > 0U) && (index < count) && (Gap_RemoveBond(index) == gBleSuccess_c))
                {
                    shell_write("\r\n-->  Bond removed!\r\n");
                    result = kStatus_SHELL_Success;
                }
...
static void ShellGap_ParseScannedDevice(gapScannedDevice_t *pData)
{
    ...
    FLib_MemCpy(mScannedDevices[mScannedDevicesCount].aAddress, pData->aAddress, sizeof(bleDeviceAddress_t));
...
static shell_status_t ShellGap_StartExtAdvertising(uint8_t argc, char *argv[])
{
    ...
    if (gBleSuccess_c != BluetoothLEHost_StartExtAdvertising(&mAppExtAdvParams, ShellGap_AdvertisingCallback,
                                                             ShellGap_ConnectionCallback))
    {
        shell_write(mShellErrorStatus);
        result = kStatus_SHELL_Success;
...
static shell_status_t ShellGap_StopExtAdvertising(uint8_t argc, char *argv[])
{
    ...
    if ((gBleSuccess_c != Gap_StopExtAdvertising(gExtAdvParams.handle)) || (mAdvertisingCbSet == FALSE))
    {
        shell_write(mShellErrorStatus);
        result = kStatus_SHELL_Success;
    }
    ...
    if (argc > 0U)
    {
        ...
        else
        {
            if (argc == 2U)
            {
                ...
                gapAdType_t advType = (gapAdType_t)BleApp_atoi(argv[0]);
                (void)ShellGap_AppendAdvData(&gAppExtAdvertisingData, (gapAdType_t)advType, argv[1], FALSE, TRUE,
                                             FALSE);
                result = kStatus_SHELL_Success;
            }
            else
...
static shell_status_t ShellGap_AddDecisionInstruction(uint8_t argc, char *argv[])
{
    ...
    else
    {
        ...
        for (uint32_t i = 0U; i < argc; i += 2U)
        {
            ...
            {
                gaDecisionInstructions[gNumDecisionInstructions].testGroup =
                    (gapDecisionInstructionsTestGroup_t)BleApp_atoi(argv[i + 1U]);
                bValidCmd = TRUE;
            }

            if (0 == strcmp((char *)argv[i], "-criteria") && ((i + 1U) < argc))
            {
                gaDecisionInstructions[gNumDecisionInstructions].passCriteria =
                    (gapDecisionInstructionsTestPassCriteria_t)BleApp_atoi(argv[i + 1U]);
                bValidCmd = TRUE;
            }

            if (0 == strcmp((char *)argv[i], "-field") && ((i + 1U) < argc))
            {
                gaDecisionInstructions[gNumDecisionInstructions].relevantField =
                    (gapDecisionInstructionsRelevantField_t)BleApp_atoi(argv[i + 1U]);
                bValidCmd = TRUE;
            }
            ...
            if (0 == strcmp((char *)argv[i], "-restagkey") && ((i + 1U) < argc))
            {
                ...
                FLib_MemCpy(&gaDecisionInstructions[gNumDecisionInstructions].testParameters.resolvableTagKey,
                            argv[i + 1U], (sizeof(uint8_t) * paramSize));
                bValidCmd = TRUE;
            }
            ...
            {
                uint8_t paramSize = BleApp_ParseHexValue(argv[i + 1U]);
                paramSize         = paramSize > gcDecisionInstructionsArbitraryDataMaskSize_c
            ...
            {
                uint8_t paramSize = BleApp_ParseHexValue(argv[i + 1U]);
                paramSize         = paramSize > gcDecisionInstructionsArbitraryDataTargetSize_c
            ...
            {
                gaDecisionInstructions[gNumDecisionInstructions].testParameters.rssi.min =
                    (int8_t)BleApp_atoi(argv[i + 1U]);
                bValidCmd = TRUE;
            }

            if (0 == strcmp((char *)argv[i], "-rssimax") && ((i + 1U) < argc))
            {
                gaDecisionInstructions[gNumDecisionInstructions].testParameters.rssi.max =
                    (int8_t)BleApp_atoi(argv[i + 1U]);
                bValidCmd = TRUE;
            }
            ...
            {
                gaDecisionInstructions[gNumDecisionInstructions].testParameters.pathLoss.min =
                    (uint8_t)BleApp_atoi(argv[i + 1U]);
                bValidCmd = TRUE;
            }

            if (0 == strcmp((char *)argv[i], "-lossmax") && ((i + 1U) < argc))
            {
                gaDecisionInstructions[gNumDecisionInstructions].testParameters.pathLoss.max =
                    (uint8_t)BleApp_atoi(argv[i + 1U]);
                bValidCmd = TRUE;
            }
            ...
            {
                gaDecisionInstructions[gNumDecisionInstructions].testParameters.advA.check =
                    (gapDecisionInstructionsAdvAChecks_t)BleApp_atoi(argv[i + 1U]);
                bValidCmd = TRUE;
            }

            if (0 == strcmp((char *)argv[i], "-add1type") && ((i + 1U) < argc))
            {
                gaDecisionInstructions[gNumDecisionInstructions].testParameters.advA.address1Type =
                    (bleAddressType_t)BleApp_atoi(argv[i + 1U]);
                bValidCmd = TRUE;
            }
            ...
            {
                gaDecisionInstructions[gNumDecisionInstructions].testParameters.advA.address2Type =
                    (bleAddressType_t)BleApp_atoi(argv[i + 1U]);
                bValidCmd = TRUE;
            }
            ...
            {
                gaDecisionInstructions[gNumDecisionInstructions].testParameters.advMode =
                    (gapDecisionInstructionsAdvMode_t)BleApp_atoi(argv[i + 1U]);
                bValidCmd = TRUE;
            }
        }
        /* A valid command should set at least one scanning parameter */
...
static shell_status_t ShellGap_RSSIMonitor(uint8_t argc, char *argv[])
{
    ...
    if (mRSSIMonitor == FALSE)
    {
        ...
        if (peerId == gInvalidDeviceId_c)
        {
            ...
            (void)Gap_StartScanning(&gAppScanParams, ShellGap_ScanningCallback, gGapDuplicateFilteringDisable_c,
                                    mScanDuration, mScanPeriod);
            result = kStatus_SHELL_Success;
        }
        else
        {
            ...
            if ((argc != 1U) && (0 == strcmp((char *)argv[1], "-c")))
            {
                ...
                if (kStatus_TimerSuccess == TM_Open((timer_handle_t)mBleShellGapTimerId))
                {
                    ...
                    mRSSIMonitor = TRUE;
                    (void)TM_InstallCallback((timer_handle_t)mBleShellGapTimerId, ShellGap_ReadRssiTmrCb,
                                             (uint8_t *)((uint32_t)peerId));
...
void ShellGap_AdvertisingCallback(gapAdvertisingEvent_t *pAdvertisingEvent)
{
    ...
    default:; /* Other Advertising Event */
        break;
    }
...
void ShellGap_ConnectionCallback(deviceId_t peerDeviceId, gapConnectionEvent_t *pConnectionEvent)
{
    ...
    default:; /* Other Connection Event */
        break;
    }
}
...
void ShellGap_ScanningCallback(gapScanningEvent_t *pScanningEvent)
{
    ...
    case gDeviceScanned_c:
    {
        if (mRSSIMonitor)
        {
            ...
            {
                FLib_MemCpyReverseOrder(&peerAddr, &pScanningEvent->eventData.extScannedDevice.aAddress,
                                        gcBleDeviceAddressSize_c);
                rssi = pScanningEvent->eventData.extScannedDevice.rssi;
            }
            else
            {
                FLib_MemCpyReverseOrder(&peerAddr, &pScanningEvent->eventData.scannedDevice.aAddress,
                                        gcBleDeviceAddressSize_c);
 ...
 ********************************************************************************** */
static void ShellGap_CopyGenericExtAdvData(uint8_t *pStart, uint32_t sizeLeftToWrite)
{
...
static void ShellGap_HandleConnectedEvt(deviceId_t peerDeviceId, gapConnectedEvent_t *pEvent)
{
    ...
    if ((mIsBonded) && (gBleSuccess_c == Gap_LoadCustomPeerInformation(peerDeviceId, NULL, 0, 0)))
    {
    ...
    FLib_MemCpy(&mConnectionParams, &pEvent->connParameters, sizeof(gapConnectionParameters_t));
}
...
static void ShellGap_HandleLongTermKeyRequestEvt(deviceId_t peerDeviceId, gapLongTermKeyRequestEvent_t *pEvent)
{
    if (pEvent->ediv == gSmpKeys.ediv && pEvent->randSize == gSmpKeys.cRandSize)
    {
...
static void ShellGap_HandlePairingCompleteEvt(deviceId_t peerDeviceId, gapPairingCompleteEvent_t *pEvent)
{
    if (pEvent->pairingSuccessful)
    {
        ...
        (void)Gap_SaveDeviceName(peerDeviceId, mScannedDevices[mConnectToDeviceId].name,
                                 (uint8_t)strlen((char *)mScannedDevices[mConnectToDeviceId].name));
    }
    else
    {
