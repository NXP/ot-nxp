...
static void   ShellGatt_DiscoveryHandler(deviceId_t serverDeviceId, gattProcedureType_t procedureType);
...
static const gattCmds_t mGattShellCmds[mShellGattCmdsCount_c] = {
    {"discover", ShellGatt_Discover}, {"read", ShellGatt_Read},     {"write", ShellGatt_WriteRsp},
    {"writecmd", ShellGatt_WriteCmd}, {"notify", ShellGatt_Notify}, {"indicate", ShellGatt_Indicate}};

const gattUuidNames_t mGattServices[] = {{0x1800, "Generic Access"},
...
const gattUuidNames_t mGattChars[] = {{0x2A00, "Device Name"},
                                      {0x2A01, "Appearance"},
...
const gattUuidNames_t mGattDescriptors[] = {{0x2900, "Characteristic Extended Properties"},
 ...
 ********************************************************************************** */
void ShellGatt_ClientCallback(deviceId_t            serverDeviceId,
 ...
 ********************************************************************************** */
void ShellGatt_ServerCallback(deviceId_t deviceId, gattServerEvent_t *pServerEvent)
{
}
 ...
 ********************************************************************************** */
void ShellGatt_NotificationCallback(deviceId_t serverDeviceId,
 ...
 * \param[in]    valueLength                  Length of characteristic's value
 ********************************************************************************** */
void ShellGatt_IndicationCallback(deviceId_t serverDeviceId,
                                  uint16_t   characteristicValueHandle,
                                  uint8_t   *aValue,
                                  uint16_t   valueLength)
{
        ...
        if (ShellGatt_CheckProcedureFinished())
        {
            ...
            case 2U:
            {
                if (0 == strcmp((char *)argv[1], "-all"))
                {
                    ...
                    mpCharDescriptorBuffer =
                        MEM_BufferAlloc(sizeof(gattAttribute_t) * mMaxServiceCharCount_d * mMaxCharDescriptorsCount_d);

                    if ((NULL == mpServiceDiscoveryBuffer) || (NULL == mpCharBuffer) ||
                    ...
                    else
                    {
                        ...
                        (void)GattClient_DiscoverAllPrimaryServices(peerId, mpServiceDiscoveryBuffer,
                                                                    mMaxServicesCount_d, &mcPrimaryServices);
            ...
            case 3U:
            {
                if (0 == strcmp((char *)argv[1], "-service"))
                {
                    ...
                    {
                        (void)GattClient_DiscoverPrimaryServicesByUuid(peerId, gBleUuidType16_c, &uuid16,
                                                                       mpServiceDiscoveryBuffer, mMaxServicesCount_d,
                                                                       &mcPrimaryServices);
                        result = kStatus_SHELL_Success;
                    }
                }
    ...
    else
    {
        ...
        else
        {
            ...
            else
            {
                ...
                (void)GattClient_WriteCharacteristicValue(peerId, mpCharBuffer, length, (uint8_t *)argv[2], fNoRsp,
                                                          FALSE, FALSE, NULL);
                (void)MEM_BufferFree(mpCharBuffer);
                mpCharBuffer = NULL;
            }
...
static shell_status_t ShellGatt_Notify(uint8_t argc, char *argv[])
{
    ...
    else
    {
        ...
        else if (gBleSuccess_c == Gap_CheckNotificationStatus(peerId, hCccd, &isNotificationActive) &&
...
static shell_status_t ShellGatt_Indicate(uint8_t argc, char *argv[])
{
    ...
    else
    {
        ...
        else if (gBleSuccess_c == Gap_CheckIndicationStatus(peerId, hCccd, &isIndicationActive) &&
...
static void ShellGatt_DiscoveryHandler(deviceId_t serverDeviceId, gattProcedureType_t procedureType)
{
    ...
    case gGattProcDiscoverAllPrimaryServices_c:
    {
        ...
        if (0U != mcPrimaryServices)
        {
            ...
            (void)GattClient_DiscoverAllCharacteristicsOfService(serverDeviceId, mpServiceDiscoveryBuffer,
            ...
            while (mCurrentCharInDiscoveryIndex < pCurrentService->cNumCharacteristics - 1U)
            {
                ...
                if (pCurrentChar->value.handle + 2U < (pCurrentChar + 1U)->value.handle)
                {
                    pCurrentChar->aDescriptors =
                        mpCharDescriptorBuffer + mCurrentCharInDiscoveryIndex * mMaxCharDescriptorsCount_d;
                    (void)GattClient_DiscoverAllCharacteristicDescriptors(serverDeviceId, pCurrentChar,
            ...
            if (bDiscoveryStarted == FALSE)
            {
                ...
                if (pCurrentChar->value.handle < pCurrentService->endHandle)
                {
                    ...
                    (void)GattClient_DiscoverAllCharacteristicDescriptors(
                        serverDeviceId, pCurrentChar, pCurrentService->endHandle, mMaxCharDescriptorsCount_d);
                    bDiscoveryStarted = TRUE;
                }
            }
        }
        ...
        if (bDiscoveryStarted == FALSE)
        {
            ...
            if (mCurrentServiceInDiscoveryIndex < mcPrimaryServices)
            {
                ...
                (void)GattClient_DiscoverAllCharacteristicsOfService(
                    serverDeviceId, mpServiceDiscoveryBuffer + mCurrentServiceInDiscoveryIndex, mMaxServiceCharCount_d);
            }
            else
            {
                ShellGatt_DiscoveryFinished();
...
static bool_t ShellGatt_CheckProcedureFinished(void)
{
    return ((mpServiceDiscoveryBuffer == NULL) && (mpCharBuffer == NULL) && (mpCharDescriptorBuffer == NULL));
}
