...
*************************************************************************************
************************************************************************************/
static const gattDbCmds_t mGattDbShellCmds[mShellGattDbCmdsCount_c] = {{"read", ShellGattDb_Read},
                                                                       {"write", ShellGattDb_Write},
                                                                       {"addservice", ShellGattDb_AddService},
                                                                       {"erase", ShellGattDb_Erase}};

static const char *mGattDbStatus[] = {"NoError",      "InvalidHandle",   "CharacteristicNotFound",
                                      "CccdNotFound", "ServiceNotFound", "DescriptorNotFound"};
...
static bleResult_t ShellGattDb_AddServiceInDatabase(serviceInfo_t *pServiceInfo)
{
    ...
    /* Add service */
    result = GattDbDynamic_AddPrimaryServiceDeclaration(0, gBleUuidType16_c, (void *)&pServiceInfo->uuid16, &handle);
    ...
    for (uint8_t i = 0; i < pServiceInfo->nbOfCharacteristics; i++)
    {
        ...
        result = GattDbDynamic_AddCharacteristicDeclarationAndValue(
            gBleUuidType16_c, (void *)&pCharacteristicInfo->uuid16, pCharacteristicInfo->properties,
            pCharacteristicInfo->maxValueLength, pCharacteristicInfo->valueLength, pCharacteristicInfo->pValue,
            pCharacteristicInfo->accessPermissions, &handle);
        if (result != gBleSuccess_c)
        {
            return result;
        }
        ...
        for (uint8_t j = 0; j < pCharacteristicInfo->nbOfDescriptors; j++)
        {
            ...
            result = GattDbDynamic_AddCharDescriptor(gBleUuidType16_c, (void *)&pDescriptorInfo->uuid16,
                                                     pDescriptorInfo->valueLength, pDescriptorInfo->pValue,
                                                     pDescriptorInfo->accessPermissions, &handle);
            if (result != gBleSuccess_c)
            {
                return result;
            }
